...
for( habitat_attribute_x in names(Habitat_Attributes_List_OKANOGAN) ){
  ...
  for(data_source_x in data_sources_x){
    ...
    # --------------------------------------
      ...
      habitat_raw_data_output_x = habitat_raw_data_output_x[,c("ReachName","score")]  # previously pulled "metric_data"
...
for(habitat_attribute_x in habitat_attribute_list){
  ...
  if(length(data_sources_combined_x) == 0){
    Okanogan_Habitat_Quality_Output_X$Data_Sources = paste("no data sources for: ",data_source_x)
...
for(habitat_attribute_x in HQ_columns){
  ...
  Habitat_Quality_Scores_Okanogan = merge(Habitat_Quality_Scores_Okanogan,habitat_attribute_scores_x, by="ReachName", all.y=TRUE )
  
  # ----------- get average Riparian score (averages the previous two) ------
...
if(Okanogan_direct_data_NOT_EDT){
  ...
  # -------------------- calculate the sum -------------
  HQ_sum_Okanogan_x = rowSums(Habitat_Quality_Scores_Okanogan[,HQ_Score_Columns])
  # -------------- merge with data frame ----------
  Habitat_Quality_Scores_Okanogan$HQ_Sum = HQ_sum_Okanogan_x # other basins: rowSums(Habitat_Quality_Scores_Okanogan[,HQ_Score_Columns]) 
  Habitat_Quality_Scores_Okanogan$HQ_Pct = HQ_sum_Okanogan_x/(5*length(HQ_Score_Columns)) 
  
# --------------- IF just using EDT data -------  
}else{
...
for(RTT_life_stage_x in unique_RTT_life_stages){
  ...
  # ---------------- loop through each EDT life stage ------------
      ...
      # ------------------ if there is a level 2 habitat attribute --------------
        ...
        for(level2_habitat_attribute_x in unique_level2_habitat_attributes_x){
          ...
          if( any(Level2_Data_Sources_Name_Crosswalk$EDT_Level_2_habitat_attribute == level2_habitat_attribute_x) ){
            ...
            if(Okanogan_direct_data_NOT_EDT){
              col_attribute_long_x = Level2_Data_Sources_Name_Crosswalk$EDT_Level_2_names_updated[col_attribute_short_x]
            }
...
# ---------------------------------------------------------------------------
#
#  Generate LF Score for each life stage 
...
LF_Scores_Okanogan_FUNCTION = function(Life_stage_Okanogan_data_frame, life_stage_x){
  ...
  LF_Scores_Output = c()
  reaches_unique_all = unique(Life_stage_Okanogan_data_frame$ReachName)
  
  for(reach_x in reaches_unique_all){
    
    # ------------------ reach data frame ---------------
    Limiting_Factor_Life_Stage_Table_REACH = Life_stage_Okanogan_data_frame[which(Life_stage_Okanogan_data_frame$ReachName == reach_x), ]
    ...
    Level_2_attributes_unique = unique(Limiting_Factor_Life_Stage_Table_REACH_numeric$Level_2_Habitat_Attribute )
    
    combined_RTT_scores = c()
    for(level_2_attributes_x in Level_2_attributes_unique){
      indiv_score = min(Limiting_Factor_Life_Stage_Table_REACH_numeric$Functional_Condition_Score[ Limiting_Factor_Life_Stage_Table_REACH_numeric$Level_2_Habitat_Attribute ==level_2_attributes_x] )
      combined_RTT_scores = c(combined_RTT_scores,indiv_score)
    }
    combined_RTT_scores = as.numeric(combined_RTT_scores)
    ...
    # ------ pull 1s -----
    x1 = which(Limiting_Factor_Life_Stage_Table_REACH$Functional_Condition_Score == 1)
    if(length(x1) > 0){
      x1_attribute = paste(Limiting_Factor_Life_Stage_Table_REACH$Level_2_Habitat_Attribute[x1], collapse=",")
    ...
    # ------ pull 3s -----
    x3 = which(Limiting_Factor_Life_Stage_Table_REACH$Functional_Condition_Score <=3 & Limiting_Factor_Life_Stage_Table_REACH$Functional_Condition_Score > 1)
    if(length(x3) > 0){
      x3_attribute = paste(Limiting_Factor_Life_Stage_Table_REACH$Level_2_Habitat_Attribute[x3], collapse=",")
...
# --------------- prep life stages output data ---------
Adult_Migration_LF_Okanogan_LF_Scores = LF_Scores_Okanogan_FUNCTION(Adult_Migration_LF_Okanogan, "Adult Migration" )
Fry_LF_Okanogan_LF_Scores  = LF_Scores_Okanogan_FUNCTION(Fry_LF_Okanogan, "Fry")
Holding_and_Maturation_LF_Okanogan_LF_Scores  = LF_Scores_Okanogan_FUNCTION(Holding_and_Maturation_LF_Okanogan, "Holding and Maturation")
Smolt_Outmigration_LF_Okanogan_LF_Scores  = LF_Scores_Okanogan_FUNCTION(Smolt_Outmigration_LF_Okanogan, "Smolt Outmigration")
Spawning_and_Incubation_LF_Okanogan_LF_Scores  = LF_Scores_Okanogan_FUNCTION(Spawning_and_Incubation_LF_Okanogan, "Spawning and Incubation")
Summer_Rearing_LF_Okanogan_LF_Scores  = LF_Scores_Okanogan_FUNCTION(Summer_Rearing_LF_Okanogan, "Summer Rearing")
Winter_Rearing_LF_Okanogan_LF_Scores  = LF_Scores_Okanogan_FUNCTION(Winter_Rearing_LF_Okanogan, "Winter Rearing")

Okanogan_Limiting_Factor_Scores_combined_ALL_REACHES = rbind(
  Adult_Migration_LF_Okanogan_LF_Scores,
  Fry_LF_Okanogan_LF_Scores,
  Holding_and_Maturation_LF_Okanogan_LF_Scores,
  Smolt_Outmigration_LF_Okanogan_LF_Scores,
  Spawning_and_Incubation_LF_Okanogan_LF_Scores,
  Summer_Rearing_LF_Okanogan_LF_Scores,
  Winter_Rearing_LF_Okanogan_LF_Scores
)
# ---------------------------------------------------------------------------
#
...
for(RTT_life_stage_x in unique_RTT_life_stages){
  ...
  # --------------- add life stage presence and priority ---------------
  #AU_level_data_x  = Life_Stage_Priorities_AU_and_Reach_data
  #Okanogan_Basic_Reach_Info_LF_initial_RTT = merge(Okanogan_Basic_Reach_Info_LF_initial_RTT, 
  ...
  for(habitat_attribute_x in habitat_attributes_life_stage_x$`Habitat Attribute`){
    ...
    # ------------ add AU priorities and presence ------------
...
test_x = FALSE
if(test_x){
  Limiting_Factor_Life_Stage_Table = Limiting_Factor_Life_Stage_Table
  # life_stage_x = "Winter Rearing" 
}
...
Generate_Species_Output_Table_Okanogan_Restoration_Protection_NO_LEVEL_3 = function(life_stage_x, Limiting_Factor_Life_Stage_Table){
  ...
  # ------------------ only pull core metrics -------------------
  Attribute_LifeStage_Crosswalk_Life_Stage_Core_Metrics = Attribute_LifeStage_Crosswalk_Life_Stage[which(Attribute_LifeStage_Crosswalk_Life_Stage$`Life Stage Core Metric?` == "x"), ]
  ...
  for(reach_x in reaches_unique_all){
    ...
    # -------------------- pull values JUST from core metrics --------------
    mylist = list(a = Limiting_Factor_Life_Stage_Table_REACH_numeric$Habitat_Attribute, b = Attribute_LifeStage_Crosswalk_Life_Stage_Core_Metrics$`Habitat Attribute`)  #OR  mylist = mget(c("a", "b", "d"))
    common_values = Reduce(intersect, mylist)
    Limiting_Factor_Life_Stage_Table_REACH_numeric_core_metric = Limiting_Factor_Life_Stage_Table_REACH_numeric[lapply(mylist, function(x) which(x %in% common_values))$a, ]
    Limiting_Factor_Life_Stage_Table_REACH_numeric_core_metric = Limiting_Factor_Life_Stage_Table_REACH_numeric_core_metric[!is.na(Limiting_Factor_Life_Stage_Table_REACH_numeric_core_metric$Habitat_Attribute),]
    # -------------- pull unique habitat attributes in this Reach ----------------
    RTT_attributes_unique = unique(Limiting_Factor_Life_Stage_Table_REACH_numeric_core_metric$Habitat_Attribute )
    
    combined_RTT_scores = c()
    for(RTT_attributes_unique_x in RTT_attributes_unique){
      indiv_score = mean(Limiting_Factor_Life_Stage_Table_REACH_numeric_core_metric$Habitat_Attribute_Score[ Limiting_Factor_Life_Stage_Table_REACH_numeric_core_metric$Habitat_Attribute ==RTT_attributes_unique_x] )
      combined_RTT_scores = c(combined_RTT_scores,indiv_score)
    }
...
Adult_Migration_LF_Okanogan_combo = Generate_Species_Output_Table_Okanogan_Restoration_Protection_NO_LEVEL_3("Adult Migration",  Adult_Migration_LF_Okanogan )
